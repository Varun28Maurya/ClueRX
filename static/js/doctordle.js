...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (activeRecord) {
    ...
    if (requestedTestId) setupArchiveQuestionNav(records, activeRecord);
  }
  ...
  function setupArchiveQuestionNav(records, activeRecord) {
    const card = document.getElementById('game-card');
    const title = card ? card.querySelector('.card-title') : null;
    if (!card || !title || records.length <= 1) return;

    const activeIndex = records.findIndex(record => record.id === activeRecord.id);
    ...
    prevWrap.className = 'archive-card-nav-btn-wrap archive-card-nav-prev-wrap';

    const titleWrap = document.createElement('div');
    titleWrap.className = 'archive-card-header-text';

    const nextWrap = document.createElement('div');
    nextWrap.className = 'archive-card-nav-btn-wrap archive-card-nav-next-wrap';

    const prevButton = createArchiveQuestionButton('prev', activeIndex - 1, records);
    const nextButton = createArchiveQuestionButton('next', activeIndex + 1, records);

    prevWrap.appendChild(prevButton);
    nextWrap.appendChild(nextButton);
    title.parentNode.insertBefore(header, title);
    titleWrap.appendChild(title);
    header.append(prevWrap, titleWrap, nextWrap);
  }

  function createArchiveQuestionButton(direction, targetIndex, records) {
    const button = document.createElement('button');
    const isPrevious = direction === 'prev';
    const targetRecord = records[targetIndex];

    button.type = 'button';
    button.className = `archive-card-nav-btn archive-card-nav-${direction}`;
    button.innerHTML = isPrevious ? '&#8592;' : '&#8594;';

    if (!targetRecord) {
      button.disabled = true;
      button.setAttribute('aria-disabled', 'true');
      button.setAttribute('aria-label', isPrevious ? 'No previous question' : 'No next question');
      return button;
    }

    button.setAttribute('aria-label', isPrevious ? 'Previous question' : 'Next question');
    button.addEventListener('click', () => {
      window.location.href = `/index.html?test=${encodeURIComponent(targetRecord.id)}`;
    });
      ...
      if (gameCompleted && gameState.gameResult) {
        ...
        setTimeout(() => {
          ...
          if (gameState.gameResult.won) {
            const winMessage = createResultNotice(`&#127881; Correct! The drug was ${diseaseName}.`, 'win');
            ...
            const lossMessage = createResultNotice(`&#10060; Game Over! The drug was ${diseaseName}.`, 'loss');
            // Only show percentiles for daily games, not archives
            const showPercentiles = !isArchiveMode;
  ...
  function updateGuessButtonLabel() {
    ...
    submitBtn.textContent = 'Submit';
    submitBtn.disabled = !selectedDiseaseId;
    submitBtn.style.opacity = selectedDiseaseId ? "1" : "0.6";
    submitBtn.style.cursor = selectedDiseaseId ? "pointer" : "not-allowed";
  }

  function createResultNotice(message, type) {
    return `<div class="result-notice result-notice-${type}"><strong>${message}</strong></div>`;
  }
  ...
  function openModal(message, shareText = null, showStats = false, guessCount = null, won = true) {
    ...
    if (showStats) {
      ...
      let archiveMessage = '';
      if (won) {
        archiveMessage = createResultNotice(`You got Doctordle #${archiveNumber} in ${guessCount} ${guessCount === 1 ? 'guess' : 'guesses'}!`, 'win');
      } else {
        archiveMessage = createResultNotice('Better luck next time!', 'loss');
      }
      ...
      fetchPercentileRanking(guessCount, won).then(percentileData => {
        if (percentileData && percentileData.tier) {
          ...
          if (percentileData.tier === "not in top 50%") {
            ...
            message = `\u{1f9e0} You were in the ${percentileData.tier} of players today!`;
            bgColor = "#e6f3ff";
            borderColor = "#2196F3";
          }

          const percentileDiv = `<div class="result-notice" style="background-color: ${bgColor}; border-color: ${borderColor};">
  ...
  if (summaryBtn) {
    summaryBtn.addEventListener('click', () => {
      ...
      summaryBtn.textContent = flipped ? '\u2190 Clues' : '+ Drug Summary';
    });
  }
  ...
  function guessing() {
    ...
    const myGuessId = selectedDiseaseId;

    if (!myGuessId) {
      resultEl.textContent = "Choose a drug from the list first.";
    ...
    if (guessedDiseases.includes(myGuessId)) {
    ...
    // Increment guess count only after a selected drug is submitted.
    guessNumber += 1;
    ...
    if (myGuessId && myGuessId == doctordle) {
      ...
      if (guessNumber >= 6) {
        ...
        gameCompleted = true;
        guessHistory.push({ name: guessName, result: 'wrong' });
        renderInlineGuessHistory();
        ...
        const lossMessage = createResultNotice(`&#10060; Game Over! The drug was ${name}.`, 'loss');
        // Only show stats for daily games, not archives
  ...
  input.addEventListener('input', function () {
    ...
    diseases.forEach(disease => {
      ...
      if (!diseaseName.includes(query) || seenNames.has(diseaseName)) return;

      const optionId = isArchiveMode && activeRecord && diseaseName === activeRecord.answer.toLowerCase()
        ? activeRecord.id
        : disease.id;

      matches.push({ id: optionId, name: disease.name });
      seenNames.add(diseaseName);
    });
